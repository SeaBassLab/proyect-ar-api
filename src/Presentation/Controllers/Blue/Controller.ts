...
import Dto from '../../../Domain/Entities/Blue/Dto'
import ObjInterface from '../../../Domain/Entities/Blue/Interface'
import Serviceable from '../../../Domain/Entities/Blue/Ports/Serviceable'
...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Validable) @named(TYPES.Blue) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Blue) private schema: Schemable
	@inject(TYPES.BlueServiceableDomain) private service: Serviceable
		...
		await this.service.save(objData, model, id)
			.then(async (res: DomainResponseable) => {
				if(res && res.result !== undefined) {
					...
					await this.service.getAll(model, {}, { operationType: { $ne: 'D' } }, {}, {}, 0, 0)
						.then((res: DomainResponseable) => {
							if(res.result.length > 0) {
								var wb = XLSX.utils.book_new();
								let ws = XLSX.utils.json_to_sheet(res.result)
