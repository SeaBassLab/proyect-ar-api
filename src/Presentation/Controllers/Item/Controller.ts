...
import Dto from '../../../Domain/Entities/Item/Dto'
...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Schemable) @named(TYPES.Purchase) private purchaseSchema: Schemable
	@inject(TYPES.Schemable) @named(TYPES.Sale) private saleSchema: Schemable
	@inject(TYPES.Schemable) @named(TYPES.Information) private informationSchema: Schemable
	...
	private getAllObjsWithAuth = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		const itemModel: Model<Document, {}> = await this.connectionProvider.getModel(db, this.schema.name, this.schema)
		const purchaseModel: Model<Document, {}> = await this.connectionProvider.getModel(db, this.purchaseSchema.name, this.purchaseSchema)
		const saleModel: Model<Document, {}> = await this.connectionProvider.getModel(db, this.saleSchema.name, this.saleSchema)
		...
		if (!error) {
			await this.service.getAll(itemModel, purchaseModel, saleModel, project, match, sort, group, limit, skip)
	...
	private getAllObjs = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		const itemModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.schema.name, this.schema)
		const purchaseModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.purchaseSchema.name, this.purchaseSchema)
		const informationModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.informationSchema.name, this.informationSchema)
		...
		if (!error) {
			await this.service.getAll(itemModel, purchaseModel, informationModel, project, match, sort, group, limit, skip)
				.then((res: DomainResponseable) => {
					if(res && res.result !== undefined) {
						if(res.result) {
							this.responserService.res = {
