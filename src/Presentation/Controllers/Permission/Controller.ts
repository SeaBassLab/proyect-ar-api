...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Validable) @named(TYPES.Permission) private dto: Validable
	...
	private getBase = async (request: RequestWithUser, response: Response, next: NextFunction) => {
	...
	private getByUser = async (request: Request, response: Response, next: NextFunction) => {

		let database: any = ''
		if(request.query.database !== undefined) database = request.query.database
		const model: Model<Document, {}> = await this.connectionProvider.getModel(database, this.schema.name, this.schema)
		const userModel: Model<Document, {}> = await this.connectionProvider.getModel(database, this.userSchema.name, this.userSchema)

		let recivedUser: any = ''

		if(request.query !== undefined) recivedUser = request.query.user

		await this.service.getByUser(model, userModel, recivedUser)
