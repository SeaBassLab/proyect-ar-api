...
import ObjInterface from "../../../Domain/Entities/Information/Interface";
import Serviceable from "../../../Domain/Entities/Information/Ports/Serviceable";
...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Validable) @named(TYPES.Information) private dto: Validable;
	@inject(TYPES.Schemable) @named(TYPES.Information) private schema: Schemable;
	@inject(TYPES.Schemable) @named(TYPES.Strain) private strainSchema: Schemable;
	@inject(TYPES.Schemable) @named(TYPES.User) private userSchema: Schemable;
	@inject(TYPES.InformationServiceableDomain) private service: Serviceable;
	@inject(TYPES.StrainServiceableDomain) private strainService: Serviceable;
	...
	initializeRoutes(validationProvider: Validateable) {
		this.router
			.get(`${this.path}/withauth/:db`, [], this.getAllObjsWithAuth)
			...
			.post(`${this.path}/withauth/:db`, [validationProvider.validate(this.dto)], this.saveObjWithAuth)
			.post(this.path, [this.authMid.authenticate, validationProvider.validate(this.dto)], this.saveObj)
			.put(`${this.path}/withauth/:db/:id`, [validationProvider.validate(this.dto, true)], this.updateObjWithAuth)
			...
			.delete(`${this.path}/withauth/:db/:id`, [], this.deleteObjWithAuth);
	}
	...
	private getAllObjsWithAuth = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		const db: string = request.params.db;

		const infomrationModel: Model<Document,{}> = await this.connectionProvider.getModel(db, this.schema.name, this.schema);
		const strainModel: Model<Document,{}> = await this.connectionProvider.getModel(db, this.strainSchema.name, this.strainSchema);
		...
		if (!error) {
			await this.service
				.getAll(infomrationModel, project, match, sort, group, limit, skip, strainModel)
	...
	private getAllObjs = async ( request: RequestWithUser, response: Response, next: NextFunction ) => {
		...
		const informationModel: Model< Document, {} > = await this.connectionProvider.getModel(request.database, this.schema.name, this.schema);
		const strainModel: Model< Document, {} > = await this.connectionProvider.getModel(request.database, this.strainSchema.name, this.strainSchema);
		...
		if (!error) {
			await this.service.getAll( informationModel, project, match, sort, group, limit, skip, strainModel)
				.then(async (res: DomainResponseable) => {
					if (res && res.result !== undefined) {
						if (res.result) {
	...
	private saveObjWithAuth = async (
		...
		var informationModel: Model<
			Document,
			{}
		> = await this.connectionProvider.getModel(
			db,
			this.schema.name,
			this.schema
		);
		...
		await this.userService
			.getAll(
				userModel,
				{},
				{ operationType: { $ne: "D" }, email: "nueve@copas.com" },
				{},
				{},
				0,
				0
		...
		await this.service
			.save(objData, informationModel, id)
			.then((res: DomainResponseable) => {
				if (res && res.result !== undefined) {
					this.responserService.res = {
						result: res.result,
	...
	private updateObjWithAuth = async (
		request: RequestWithUser,
		response: Response,
		next: NextFunction
	) => {
		...
		await this.service
			...
			.then(async (res: DomainResponseable) => {
				if (res && res.result !== undefined) {
					if (res.status === 200) {
						await this.service
							.update(id, objData, model, idUser)
							.then((res: DomainResponseable) => {
								if (res && res.result !== undefined) {
									this.responserService.res = {
										result: res.result,
									...
									this.responserService.res = {
										result: "Nop",
										message:
											"La capa superior contesto undefined",
										error: "",
										status: 500,
									};
								}
	...
	private deleteObjWithAuth = async (
		request: RequestWithUser,
		response: Response,
		next: NextFunction
	) => {
