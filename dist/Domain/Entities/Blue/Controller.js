                            ...
                            return _this4.geteableAllService.getAll(model, project, match, sort, group, limit, skip).then(function (result) {
                              if (result) {
                                if (result.result !== undefined && result.result !== null && result.result[0] !== undefined && result.result[0].items !== undefined && Array.isArray(result.result[0].items) && result.result[0].items.length > 0) {
                                  result.result[0].items.map(function (blue) {
                                    blue['amountExchange'] = _this4.roundTo(blue.amount * blue.exchangeRate, 2);
                                    blue['totalCommission'] = _this4.roundTo(blue.amount * blue.commission, 2);

                                    if (blue.state === 'Pagado') {
                                      if (blue.operation === 'VENTA') blue['totalClient'] = _this4.roundTo(blue['amountExchange'] - blue['totalCommission'], 2);
                                      if (blue.operation === 'COMPRA') blue['totalClient'] = _this4.roundTo(blue['amountExchange'] + blue['totalCommission'], 2);
                                    }

                                    if (blue.state === 'No Pagado') blue['totalClient'] = _this4.roundTo(blue['amountExchange'], 2); // condicional para mongo
