                            ...
                            return _this4.geteableAllService.getAll(model, project, match, sort, group, limit, skip).then(function (result) {
                              if (result) {
                                if (result.result !== undefined && result.result !== null && result.result[0] !== undefined && result.result[0].items !== undefined && Array.isArray(result.result[0].items) && result.result[0].items.length > 0) {
                                  result.result[0].items.map(function (wire) {
                                    wire['comissionClient'] = _this4.roundTo(wire.amount * wire.exchangeRate / 100, 2);

                                    if (wire.operation === 'BAJADA') {
                                      wire['finalClient'] = _this4.roundTo(wire.amount - wire['comissionClient'], 2);
                                    } else {
                                      wire['finalClient'] = _this4.roundTo(wire.amount + wire['comissionClient'], 2);
                                    }

                                    wire['cost'] = _this4.roundTo(wire.amount * wire.commission / 100, 2);
                                    wire['amountTraded'] = _this4.roundTo(wire.amount + wire['cost'], 2);
                                    wire['gain'] = _this4.roundTo(wire['comissionClient'] + wire['cost'], 2);

                                    if (wire.payToClient) {
                                      wire['balance1'] = _this4.roundTo(wire['finalClient'] + -1, 2);
                                    } else {
                                      wire['balance1'] = 0;
                                    }

                                    if (wire.payToOperator) {
                                      wire['balance2'] = _this4.roundTo(wire['amountTraded'], 2);
                                    } else {
                                      wire['balance2'] = 0;
                                    }

                                    wire['balanceTotal'] = _this4.roundTo(wire['balance1'] - wire['balance2'], 2);
                                  });
                                }
