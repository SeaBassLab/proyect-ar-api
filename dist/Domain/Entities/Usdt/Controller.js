                            ...
                            return _this4.geteableAllService.getAll(model, project, match, sort, group, limit, skip).then(function (result) {
                              if (result) {
                                if (result.result !== undefined && result.result !== null && result.result[0] !== undefined && result.result[0].items !== undefined && Array.isArray(result.result[0].items) && result.result[0].items.length > 0) {
                                  result.result[0].items.map(function (usdt) {
                                    usdt['amountExchange'] = _this4.roundTo(usdt.amount * ((100 + usdt.exchangeRate) / 100), 2);
                                    usdt['totalCommission'] = _this4.roundTo(usdt.amount * (usdt.commission / 100), 2);

                                    if (usdt.state === 'Pagado') {
                                      if (usdt.operation === 'VENTA') usdt['totalClient'] = _this4.roundTo(usdt['amountExchange'] - usdt['totalCommission'], 2);
                                      if (usdt.operation === 'COMPRA') usdt['totalClient'] = _this4.roundTo(usdt['amountExchange'] + usdt['totalCommission'], 2);
                                    }

                                    if (usdt.state === 'No Pagado') usdt['totalClient'] = _this4.roundTo(usdt['amountExchange'], 2);
